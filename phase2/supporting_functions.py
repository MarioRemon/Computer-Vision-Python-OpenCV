...
def update_rover(Rover, data):
    ...
    if Rover.start_time == None:
        ...
        # print(Rover.start_pos)
        # Rover_start_xpos = np.int_([convert_to_float(pos.strip())
        #                        for pos in data["start_pos_x"].split(';')])
        # Rover_start_ypos = np.int_([convert_to_float(pos.strip())
        #                         for pos in data["start_pos_y"].split(';')])
        # Rover.start_pos = (Rover_start_xpos,Rover_start_ypos)
        Rover.total_time = 0
        ...
        # The start position of the rover
        Rover.start_pos = [convert_to_float(pos.strip()) for pos in data["position"].split(';')]  # = (x, y)
        print("Start Position", Rover.start_pos)
    ...
    Rover.pos = [convert_to_float(pos.strip()) for pos in data["position"].split(';')]  # = (x, y)
    Rover.visited_pos.append([convert_to_float(pos.strip()) for pos in data["position"].split(';')])  # = (x, y)
    #print("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
    #print(Rover.visited_pos)
...
def create_output_images(Rover):
    # if np.max(Rover.worldmap[:, :, 3]) > 0:
    #     start_pos = Rover.worldmap[:, :, 3] > 0
    #     starto = Rover.worldmap[:, :, 3] * (255 / np.mean(Rover.worldmap[start_pos, 3]))
